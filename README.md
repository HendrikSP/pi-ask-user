...
- Scope: **only** a currently pending `ask_user` call that is still waiting inside `ctx.ui.custom(...)` or the zero-option `ctx.ui.input(...)` path
...
- stale or late `toolCallId` values after the live custom UI or zero-option input has already settled
