...
describe("ask_user", () => {
   ...
   describe("external response handler", () => {
      ...
      test("non-interactive and zero-option paths remain unaffected", async () => {
         const { tool, events } = await setupExtension();
         ...
         const zeroOptionPromise = tool.execute(
            "zero-option-call-id",
            {
         ...
         const zeroOptionResult = await zeroOptionPromise;

         expect(zeroOptionResult.details.cancelled).toBe(false);
         expect(zeroOptionResult.details.response).toEqual({ kind: "freeform", text: "Need more telemetry." });
      });
